...
def analyse():
    ...
    Endpoint principal :
    - Reçoit une image (form-data "file")
    - Paramètres envoyés par le front dans le FormData :
        country, shop, custom_shop
    - Retourne JSON :
        {
          ok: true/false,
          description: "...",
          shop: "...",          # (valeur technique utilisée dans l'URL)
          shop_label: "...",    # (ce que tu affiches dans l'UI)
          country: "...",
          url: "...",
          source: "ai+ocr" / "ocr-only" / "ai-only" / "fallback" / "cache",
          from_cache: bool
    ...
    # 2) Boutique & pays (d'abord dans form-data, sinon querystring)
    ...
    # Ce qui sera vraiment utilisé pour construire l'URL
    ...
    # 3) Cache (clé = image + boutique + pays)
    ...
    # 8) Construire l'URL de recherche (avec la boutique effective)
